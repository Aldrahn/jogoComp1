...
int selectionScreen(SDL_Window *window, SDL_Renderer *rend)
{
  	...
  	while(isRunning)
	{
		while (SDL_PollEvent(&ev) != 0)
		{
			...
			else if(retornoGameLoop == 2)
			{
				//printf("retorno: %d\n", retornoGameLoop);
				isRunning = false;
			}

			else if(retornoGameLoop == 3)
			{
				//printf("retorno: %d\n", retornoGameLoop);
				isRunning = false;
			}

			else if(retornoGameLoop == 0)
			{
				isRunning = false;
			}
...
int gameLoop(SDL_Window *window, SDL_Renderer *rend, int playerType)
{
	...
	SDL_Texture* textureWithScoring = loadFontForScoring(rend);
	//Retângulos do BG
	...
	while (isRunning)
	{
		...
		if(pause == false)
		{
			...
			{	
				if(arrayWave[i] != NULL && arrayWave[i]->doDestroyingAnimation)
				{
					...
					arrayWave[i]->frameTimeDestroy++;
					if(FPS/arrayWave[i]->frameTimeDestroy == 30)
					{
						arrayWave[i]->frameTimeDestroy = 0;
				...
				{
					arrayWave[i]->frameTimeDamage++;
					if(FPS/arrayWave[i]->frameTimeDamage == 15)
					{
						arrayWave[i]->frameTimeDamage = 0;
			...
			if(waveCounter == 11 && arrayWave[0]->enemy->hp <= 0)
			{
				free(arrayWave[0]->enemy);
				free(arrayWave[0]);
			...
			else if (spawnedIndex + 1 == waveRegister[waveCounter] && checkWaveStatus(arrayWave, spawnedIndex + 1) && waveCounter != 19)
			{
		...
		showScoreOnScreen(rend, textureWithScoring, player->points);
		SDL_RenderPresent(rend);
		SDL_UpdateWindowSurface(window);
...
void enemyDmgAnimation(EnemyShip* ship)
{
	...
	else if(ship->type == 3 || ship->type == 7 || ship->type == 9)
	{
...
EnemyShip *createEnemyShip(int enemyType, int x, int y, int movement, int upDown, int leftRight, int spawn, int distance, int stop)
{
	...
	new->frameTimeDamage = 0;
	new->frameTimeDestroy = 0;
...
Bullet *createBullet(Ship *source, BulletVector *bulletVector, int bulletType, int dist, int playerType)
{
	...
	else if(bulletType == 7 || bulletType == 9 || bulletType == 3)
	{
		bullet->speed = 9;
		bullet->damage = 10;
		bullet->x_axis = source->x_axis + (source->dstrect.w/2);
		bullet->y_axis = source->y_axis + source->dstrect.h;
		bullet->owner = source;
		SDL_Rect a = {200, 1, 36, 46};
		SDL_Rect b = {bullet->x_axis, bullet->y_axis, 38, 46};
		bullet->srcrect = a;
	...
	else if(bulletType == 4)
	{
        ...
        bullet->damage = 15;
        bullet->x_axis = source->x_axis + dist;
        bullet->y_axis = source->y_axis + 80;
        bullet->owner = source;
        SDL_Rect a = {200, 1, 36, 46};
        SDL_Rect b = {bullet->x_axis, bullet->y_axis, 38, 46};
        bullet->srcrect = a;
...
BulletVector *moveBullet(BulletVector *bulletVector, EnemyShip** arrayWave, PlayerShip* player, int spawnedIndex)
{
	...
	{	
		...
		else
		{
			...
			else if((bullet->type == 1 || bullet->type == 3 || bullet->type == 4 || bullet->type == 5 || bullet->type == 7 || bullet->type == 9) && bullet->y_axis >= 720)
			{
				free(bullet);
				bulletVector->bullets[i] = NULL;
				rearrangeBulletVector(bulletVector);
			...
			else if(bullet->type == 2 && bullet->x_axis >= 1280)
			{
				free(bullet);
				bulletVector->bullets[i] = NULL;
				rearrangeBulletVector(bulletVector);
			...
			else if(bullet->type == 6 && bullet->x_axis < 0)
			{
				free(bullet);
				bulletVector->bullets[i] = NULL;
				rearrangeBulletVector(bulletVector);
...
void changeShipColor(PlayerShip* player)
{
	if(player->type == 3)
	{
		if(player->ally->hp < 100)
		{
			player->ally->srcrect.y = 600;
		}
		else if(player->ally->hp > 100 && player->ally->hp < 200)
		{
...
EnemyShip** waveLoader(EnemyShip** arrayWave, int* waveRegister, int waveCounter)
{
	...
	for (int i = 0; i < waveRegister[waveCounter]; i++)
	{
		...
		else if(waveCounter == 5)
		{
			...
			if(i+1 < waveRegister[waveCounter])
			{
				arrayWave[i+1] = createEnemyShip(1, 1280, 300, diagonal, cima, esquerda, sequential, 0, -50);
				i++;
			}
		}	
...
void showScoreOnScreen(SDL_Renderer* renderer, SDL_Texture* textureWithScoring, int scorePlayer)
{
	...
	textureWithScoring = SDL_CreateTextureFromSurface(renderer, surfaceWithScoring);
...
SDL_Texture* loadFontForScoring(SDL_Renderer* renderer)
{
